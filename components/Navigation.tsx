...
const Navigation: React.FC<NavigationProps> = ({ ride, phase, onPrimaryAction, onChat, onCall }) => {
  ...
  const isApproaching = timeLeftSeconds < 120;

  const parseCoordinates = (value?: string): [number, number] | null => {
    if (!value) return null;
    const [lng, lat] = value.split(',').map((entry) => Number(entry.trim()));
    if (Number.isNaN(lng) || Number.isNaN(lat)) return null;
    return [lng, lat];
  };

  const fallbackCenter: [number, number] = [3.3792, 6.5244];
  const point = isPickup ? parseCoordinates(ride.pickup) : parseCoordinates(ride.dropoff);
  const [lng, lat] = point ?? fallbackCenter;

  const mapboxEmbedUrl = mapboxToken
    ? `https://api.mapbox.com/styles/v1/mapbox/navigation-day-v1.html?title=false&zoomwheel=false&fresh=true#14/${lat}/${lng}`
    : null;
      ...
      <div className="absolute inset-0 h-full w-full bg-gray-200 overflow-hidden">
        {mapboxEmbedUrl ? (
          <iframe
            ...
            src={mapboxEmbedUrl}
            className="h-full w-full border-0 transition-transform duration-[2000ms] ease-linear scale-110"
            allow="geolocation"
          />
        ) : (
          <div className="h-full w-full bg-slate-300 dark:bg-slate-800 flex items-center justify-center px-6 text-center text-sm text-slate-700 dark:text-slate-200">
            Add VITE_MAPBOX_TOKEN to enable live Mapbox navigation view.
          </div>
        )}
          ...
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 flex flex-col items-center gap-2 z-10">
            <div className="relative flex items-center justify-center">
              ...
              <div className="size-10 bg-[#137fec] border-[3px] border-white dark:border-[#1C1C1E] rounded-full shadow-xl flex items-center justify-center relative z-10">
                <span className="material-symbols-outlined text-white text-lg">{isPickup ? 'person' : 'flag'}</span>
              </div>
            </div>
            ...
            <div className="bg-white dark:bg-[#1C1C1E] px-3 py-1.5 rounded-lg shadow-lg border border-gray-100 dark:border-gray-800 animate-in fade-in slide-in-from-bottom-2">
              <p className="text-xs font-bold text-slate-900 dark:text-white whitespace-nowrap">{isPickup ? ride.passengerName : 'Destination'}</p>
              <p className="text-[10px] text-slate-500 text-center font-medium">{isPickup ? 'Passenger Location' : 'Drop-off Point'}</p>
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="flex flex-col gap-4">
          <div className="flex flex-col gap-2">
            ...
            <div className="h-2 w-full rounded-full bg-gray-100 dark:bg-gray-800 overflow-hidden">
              <div className="h-full rounded-full bg-[#FFC107] transition-all duration-1000 ease-linear" style={{ width: `${Math.min(100, progress)}%` }}></div>
            </div>
            <p className="text-sm font-medium text-slate-500 dark:text-slate-400">{distanceDisplay} {distUnit} remaining</p>
          </div>
          ...
          <div className="flex items-center gap-4">
            <div className="relative">
              <img src={ride.passengerImage} alt={ride.passengerName} className="h-14 w-14 rounded-full object-cover border-2 border-white dark:border-gray-700 shadow-sm" />
            ...
            <div className="flex-1">
              <p className="text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400 mb-0.5">{isPickup ? 'Pickup Passenger' : 'Drop-off Passenger'}</p>
            ...
            <button onClick={onCall} className="flex size-12 shrink-0 items-center justify-center rounded-full bg-gray-100 dark:bg-gray-800 text-slate-900 dark:text-white hover:bg-green-100 hover:text-green-600 transition-colors">
              <span className="material-symbols-outlined">phone</span>
            </button>
            <button onClick={onChat} className="flex size-12 shrink-0 items-center justify-center rounded-full bg-gray-100 dark:bg-gray-800 text-slate-900 dark:text-white hover:bg-blue-100 hover:text-blue-600 transition-colors">
          ...
          <button onClick={onPrimaryAction} className="group relative flex w-full items-center justify-center overflow-hidden rounded-xl bg-[#FFC107] py-4 shadow-md transition-transform active:scale-[0.98]">
